runOccuAllyearsNetworkWide<-function(DataIn, modSel, networkName,nGOF){
  require(unmarked)
  require(plyr)
  require(reshape2)
  require(AICcmodavg)
    ...
    new.data<-subset(data, Survey_Type==habitatType)
      ...
      unitName<-as.character(networkName)
        ...
        for(r in 1:length(speciesList)){
          ...
          new.umf<-NULL
          new.umf<-tryCatch({
            suppressWarnings(makePOccu_umf(DataIn=new.data, SpeciesIn=speciesName))
          ...
          #model 1 - Null
          print("Mod1")
          mod1<-tryCatch({suppressWarnings(occu(~1 ~Year, data=new.umf))
          }, error= function(out){
            out=NULL
          })
          ...
          #mod1.gof
          print("Mod1-GOF")
          mod1.gof<- try(mb.gof.test(mod1, nsim = nGOF, plot.hist = FALSE,report = NULL, parallel = FALSE))
          ...
          print("Mod2")
          mod2<-tryCatch({suppressWarnings(occu(~Observer ~Year, data=new.umf))
          }, error=function(out){
            out=NULL
          })
          ...
          #mod2.gof
          print("Mod2-GOF")
          mod2.gof<- try(mb.gof.test(mod2, nsim = nGOF, plot.hist = FALSE,report = NULL, parallel = FALSE))
          mod2.c.hat<-mod2.gof$c.hat.est
          
          
          #model 3 - Ord.Day (det)
          print("Mod3")
          mod3<-tryCatch({suppressWarnings(occu(~Ord.Day ~Year, data=new.umf))
          }, error=function(out){
            out=NULL
          })
          ...
          #mod3.gof
          print("Mod3-GOF")
          mod3.gof<- try(mb.gof.test(mod3, nsim = nGOF, plot.hist = FALSE,report = NULL, parallel = FALSE))
          mod3.c.hat<-mod3.gof$c.hat.est
          
          
          #model 4 - Observer+Ord.Day (det) 
          print("Mod4")
          mod4<-tryCatch({suppressWarnings(occu(~Observer+Ord.Day ~ Year, data=new.umf))
          }, error=function(out){
            out=NULL
          })
          ...
          #mod4.gof
          print("Mod4-GOF")
          mod4.gof<- try(mb.gof.test(mod4, nsim = nGOF, plot.hist = FALSE,report = NULL, parallel = FALSE))
          mod4.c.hat<-mod4.gof$c.hat.est
          
          #model 5 - Observer+Ord.Day+Time (det)
          print("Mod5")
          
          mod5<-tryCatch({suppressWarnings(occu(~Observer+Ord.Day+Time ~ Year, data=new.umf))
          }, error=function(out){
            out=NULL})
          ...
          mod5.gof<- try(mb.gof.test(mod5, nsim = nGOF, plot.hist = FALSE,report = NULL, parallel = FALSE))
          ...
          mod6<-tryCatch({suppressWarnings(occu(~Observer+Ord.Day+Wind ~ Year, data=new.umf))
          }, error=function(out){
            out=NULL})
          ...
          mod6.gof<- try(mb.gof.test(mod6, nsim = nGOF, plot.hist = FALSE,report = NULL, parallel = FALSE))
