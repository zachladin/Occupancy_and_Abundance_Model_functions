    ...
    for(j in 1:length(unitList)){
      ...
      for(k in 1:length(yearList)){
        ...
        for(r in 1:length(speciesList)){
          ...
          #make umf
          new.umf<-tryCatch({
            makePCount_umf(DataIn=year.data, SpeciesIn=speciesName)
          ...
          abun.out<-NULL
          ifelse(mod.name=="Null_NB",abun.out<-predict(mod1,type="state",appendData=TRUE),
                 ifelse(mod.name=="Null_P", abun.out<-predict(mod2,type="state",appendData=TRUE),
                        ifelse(mod.name=="Observer_NB", abun.out<-predict(mod3,type="state",appendData=TRUE),
                               ifelse(mod.name=="Observer_P", abun.out<-predict(mod4,type="state",appendData=TRUE),
                                      ifelse(mod.name=="Observer.Day_NB", abun.out<-predict(mod5,type="state",appendData=TRUE),
                                             ifelse(mod.name=="Observer.Day_P", abun.out<-predict(mod6,type="state",appendData=TRUE),
                                                    abun.out<-data.frame(Predicted=NA, SE=NA, lower=NA,upper=NA,y.1=NA,y.2=NA,Year=yearName,Unit_Code=unitName,Plot_Name=NA,Visit.1=NA,Visit.2=NA,Ord.Day.1=NA,Ord.Day.2=NA,Time.1=NA,Time.2=NA, Temp.1=NA,Temp.2=NA, Wind.1=NA,Wind.2=NA,Observer.1=NA,Observer.2=NA)
                                             ))))))
          ...
          #get model formula
          formula.out<-NULL
          ifelse(mod.name=="Null_NB",formula.out<-paste(deparse(mod1@call)[1],sep=""),
                 ifelse(mod.name=="Null_P", formula.out<-paste(deparse(mod2@call)[1],sep=""),
                        ifelse(mod.name=="Observer_NB", formula.out<-paste(deparse(mod3@call)[1],sep=""),
                               ifelse(mod.name=="Observer_P", formula.out<-paste(deparse(mod4@call)[1],sep=""),
                                      ifelse(mod.name=="Observer.Day_NB", formula.out<-paste(deparse(mod5@call)[1],sep=""),
                                             ifelse(mod.name=="Observer.Day_P", formula.out<-paste(deparse(mod6@call)[1],sep="") ,
                                                    formula.out<-NA
                                             ))))))
                 ...
                 ifelse(mod.name=="Null_P", abun.out<-predict(mod2,type="state",appendData=TRUE),
                        ifelse(mod.name=="Observer_NB", abun.out<-predict(mod3,type="state",appendData=TRUE),
                               ifelse(mod.name=="Observer_P", abun.out<-predict(mod4,type="state",appendData=TRUE),
                                      ifelse(mod.name=="Observer.Day_NB", abun.out<-predict(mod5,type="state",appendData=TRUE),
                                             ifelse(mod.name=="Observer.Day_P", abun.out<-predict(mod6,type="state",appendData=TRUE),
                                                    abun.out<-data.frame(Predicted=NA, SE=NA, lower=NA,upper=NA,y.1=NA,y.2=NA,y.3=NA,Year=yearName,Unit_Code=unitName,Plot_Name=NA,Visit.1=NA,Visit.2=NA,Visit.3=NA,Ord.Day.1=NA,Ord.Day.2=NA,Ord.Day.3=NA,Time.1=NA,Time.2=NA,Time.3=NA, Temp.1=NA,Temp.2=NA,Temp.3=NA, Wind.1=NA,Wind.2=NA,Wind.3=NA,Observer.1=NA,Observer.2=NA,Observer.3=NA)
                                             ))))))
